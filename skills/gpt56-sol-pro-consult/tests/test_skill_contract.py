...
class SkillContractTests(unittest.TestCase):
    ...
    def test_chrome_workflow_isolates_opencli_debugger_tabs(self) -> None:
        skill = (SKILL_DIR / "SKILL.md").read_text(encoding="utf-8")
        workflow = (SKILL_DIR / "references/chrome-workflow.md").read_text(encoding="utf-8")
        for text in (
            "both use Chrome's debugger API",
            "fresh or agent-owned tab",
            "must not replace the Chrome path",
        ):
            self.assertIn(text, skill)
        for text in (
            "browser.tabs.list()",
            "OpenCLI Browser",
            "OpenCLI is eligible only after the routing contract",
        ):
