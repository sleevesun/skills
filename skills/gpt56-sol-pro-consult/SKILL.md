...
Keep the browser paths isolated. The Codex Chrome extension and OpenCLI both use Chrome's debugger API, so never run OpenCLI commands against a tab that Codex is controlling. The default Chrome workflow must use a fresh or agent-owned tab; the text-only `run_gpt56_sol_pro_consult.py` wrapper is only the documented OpenCLI fallback and must not replace the Chrome path merely because OpenCLI is installed.
...
- **Debugger conflict or tab preemption:** create a fresh agent-owned tab and retry the preparation step once. Do not bind OpenCLI to that tab or reuse a tab in an OpenCLI-owned group. If the conflict occurs after Send, follow `NOT_SENT` / `SENT` / `UNKNOWN` recovery and never duplicate the request.
