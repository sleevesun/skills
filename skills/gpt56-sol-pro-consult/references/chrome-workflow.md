...
5. Reuse an existing ChatGPT tab only when it is already listed by `browser.tabs.list()` as an agent-owned tab; otherwise create one and navigate directly to `https://chatgpt.com/`. Do not claim a user tab from `browser.user.openTabs()` unless the user explicitly mentioned that exact tab.

6. Keep Codex and OpenCLI tabs separate. Both extensions can use Chrome's debugger API, and an OpenCLI command may preempt the debugger connection for a shared tab. Never use a tab in an `OpenCLI Browser` group or a tab bound to an OpenCLI session for this workflow. If the available ChatGPT tab is OpenCLI-owned, create a fresh agent-owned tab instead.

Do not inspect cookies, local storage, passwords, profiles, or session databases. Keep browser work in the background unless the user asks to see it. Do not invoke the text-only OpenCLI wrapper on this default path; OpenCLI is eligible only after the routing contract and its preflight both pass.
