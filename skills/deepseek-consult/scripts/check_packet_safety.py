#!/usr/bin/env python3
"""Scan a DeepSeek context packet for executable credentials."""
...
def excerpt(text: str, start: int, end: int) -> str:
    value = text[max(0, start - 12) : min(len(text), end + 12)].replace("\n", "\\n")
    ...
    for severity, patterns in (("high", HIGH_PATTERNS), ("warn", WARN_PATTERNS)):
        for kind, pattern in patterns:
            for match in pattern.finditer(text):
                findings.append({"severity": severity, "type": kind, "start": match.start(), "end": match.end(), "excerpt": excerpt(text, match.start(), match.end())})
    high_count = sum(item["severity"] == "high" for item in findings)
    warn_count = sum(item["severity"] == "warn" for item in findings)
    return {"ok": high_count == 0, "high_count": high_count, "warn_count": warn_count, "char_count": len(text), "findings": findings}


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("packet", help="Packet Markdown path, or '-' for stdin")
    ...
    if len(text) > args.max_chars:
        ...
        result["findings"].append({"severity": severity, "type": "packet_too_long", "start": args.max_chars, "end": len(text), "excerpt": f"{len(text)} chars exceeds max {args.max_chars}"})
        result["high_count" if severity == "high" else "warn_count"] += 1
        if severity == "high":
            result["ok"] = False
