...
scriptPath = fso.BuildPath(root, "targets\windows\apply.ps1")
command = "powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File " & Quote(scriptPath)
exitCode = shell.Run(command, 0, True)
If exitCode <> 0 Then
  MsgBox "WorkBuddy theme application failed. Confirm WorkBuddy is installed, then try again.", 16, "WorkBuddy Theme"
End If
